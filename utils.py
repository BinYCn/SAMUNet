...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], input_size=[224, 224],
...
def load_parameters(model, filename):
    ...
    weight_name = state_dict.keys()

    sam_dict = model.state_dict()
    sam_keys = sam_dict.keys()

    keys = [k for k in sam_keys if k in weight_name]
...
def save_parameters(model, filename):
    ...
    for key, value in state_dict.items():
        if 'a_q' in key or 'a_v' in key:
            a_tensors[key] = value
        if 'b_q' in key or 'b_v' in key:
            b_tensors[key] = value
        if 'bias' in key and 'qkv' in key:
            bias_tensors[key] = value
        if 'sam_encoder' not in key:
            other_tensors[key] = value

    merged_dict = {**a_tensors, **b_tensors, **bias_tensors, **other_tensors}
